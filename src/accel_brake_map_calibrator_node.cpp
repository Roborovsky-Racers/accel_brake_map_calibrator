...
: Node("accel_brake_map_calibrator", node_options)
{
  ...
  RCLCPP_WARN(get_logger(), "CSV calibrated map dir = %s", csv_calibrated_map_dir_.c_str());
  RCLCPP_WARN(get_logger(), "Accel map path = %s", output_accel_file_.c_str());
  RCLCPP_WARN(get_logger(), "Brake map path = %s", output_brake_file_.c_str());
